...
XPathInterpreter.prototype.getMultipleRelativeXPaths = function (
  element,
  parent,
  generatesSingleElemSelectors
) {
  var xPathArray = [];
  ...
  for (var i = 0; i < this.engine.length; i++) {
    if (
      this.engine[i].suitableForRelative() &&
      this.engine[i].generatesSingleElemSelectors() ==
        generatesSingleElemSelectors
    ) {
      try {
...
XPathInterpreter.prototype.getMultipleXPaths = function (
  element,
  parent,
  generatesSingleElemSelectors
) {
  ...
  if (parent == undefined) parent = element.ownerDocument;
...
BasicIdEngine.prototype.getPath = function (element, parent) {
  if (element && element.id) {
    return [
      './/' + element.nodeName.toLowerCase() + '[@id="' + element.id + '"]',
    ];
  } else {
    return;
  }
...
ControlTypeBasedEngine.prototype.getPath = function (element, parent) {
  ...
  while ((traversingElem = traversingElem.parentElement)) {
    if (traversingElem.className) {
      ...
      accumPathEnding =
        traversingElem.nodeName.toLowerCase() + '/' + accumPathEnding;
    }
  }
...
ClassXPathEngine.prototype.getPath = function (element, parent) {
  ...
  for (var i = 0; i < elemClasses.length; i++) {
    var elemPath =
      './/' + tagName + "[contains(@class, '" + elemClasses[i] + "')]";
  ...
  for (
    ...
    for (
      ...
      if (sibling.nodeType == 10)
        //element.ownerDocument.defaultView.Node.DOCUMENT_TYPE_NODE
        continue;
